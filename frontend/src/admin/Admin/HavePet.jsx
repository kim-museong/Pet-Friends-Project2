...
const HavePet = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        // Fetch the pets data for the user ids from the pets table
        axios
          .get(`/api/pets?userIds=${userIds.join(',')}`)
          .then((petsResponse) => {
            // Filter users whose id exists in the pets table
            const usersWithPets = sortedUsers.filter((user) => petsResponse.data.some((pet) => pet.userId === user.id));

            setUsers(usersWithPets);
            setTotalUsers(usersWithPets.length);
          })
          .catch((error) => {
  ...
  return (
    <Box m="20px">
      ...
      <PetInfoDialog isOpen={isPetInfoDialogOpen} user={selectedUser} pet={selectedPet} onClose={handlePetInfoClose} />
    </Box>
  );
};
